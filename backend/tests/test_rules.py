...
def test_rule_proposals_include_specific_single_token_merchants(db):
...
def test_rule_proposals_still_hide_generic_single_word_keys(db):
    account = _credit_account(db)
    subscriptions = _category(db, "Subscriptions")
    for _ in range(3):
...
def test_rule_proposals_group_processor_style_payment_variants(db):
    account = _credit_account(db)
